...
The resolver's walks — search the aliases, pick the candidate by episode count and year, chase the episode to a playable URL — are policy, and they read the same whichever site answers. What differs per site sits behind a `Provider` trait in `backend/src/scraper/`: how a query becomes hits, how a slug becomes an episode list, how an episode becomes a playlist — hianime's embed hosts differ even there: zokoanime's page carries the stream in a payload, megaplay's names a media id the sources endpoint on that page's own host answers for, as ciphertext under constants the site's player script carries and for whichever content delivery network the request asks about — the site can stream an episode from several and they do not all stream alike, so the client asks every megaplay page for the one whose segments it can play, whatever network the listing named that server for — and the client reads a page by its shape, asks the master before taking a server, tries megaplay's servers ahead of zokoanime's — megaplay's delivery network has carried the top rendition at real time where zokoanime's has crawled or been down, so the site's order, zokoanime first, would hand every play to the slower host while it still answers — and steps to the next server when a stream's host does not answer — and what the CDN wants on the request (hianime's playlists are served only with the origin of the host that served the embed page as `Referer`, which the site's listing does not always name, since it moves those pages between hosts with a redirect; anidb.app's need none). A resolved stream carries that context — the master URL, its referer, its sidecar subtitle tracks — and every consumer reads it from there: the proxy session, the resolution cache, downloads, and the external-player and Syncplay handoffs.

Two providers ship: anidb.app, then hianime. Each has an admission gate of its own — a pacer for background probes and a circuit breaker that opens after consecutive failures — so an outage on one never paces or refuses traffic to the other. The failover orchestrator (`backend/src/commands/providers.rs`) runs every walk against the providers in order and moves to the next only when the current one was **unreachable, refusing or broken**: a transport failure, a timeout, a provider-shaped block or rate limit, a background request the gate refused, or a page the parser no longer recognises. A provider that answered conclusively — including one that searched and found nothing — ends the walk; a miss on anidb.app is not retried on hianime. An answer that settles nothing does not: a provider that found the show but whose sampled episode rows all say nothing about the requested audio mode has answered inconclusively, and the walk moves on to the next provider as it would from an unreachable one, surfacing the unknown verdict itself when nobody gives an answer that is not negative — a clean miss or an absence from the rest does not replace it. The other exception is a show a live positive availability row already places: the row puts its provider first, and that provider's own first negative — a clean catalogue miss included — is set aside rather than ending the walk, and the rest of the order is asked anyway. What they answer then stands against the set-aside negative by rank rather than by order: a negative that found the show — an episode it does not carry, or the mode absent — outranks a title miss from another catalogue, whether the search matched nothing or matched only candidates whose episode lists were gone — except that an absence and a clean miss, which both deny the show for the mode asked and both persist, are peers — negatives of equal rank keep the one whose provider stands earliest in the configured order — the row the read rule below can serve, since a negative row is served only while every provider ahead of its own is refusing, so a remembered fallback's own miss yields to the primary's equal one and a remembered primary's stands over the fallback's — and the set-aside one stands as given when the rest were unreachable. The paragraph below works that ranking through. A provider that is refusing — its breaker open, or an advertised rate-limit window still running — is skipped while another remains, on background and interactive walks alike; the last one is always tried, since an interactive request may be the trial that closes a breaker, and a click is admitted through a pause regardless. Every attempt but the last is bounded by a 20-second budget under the 60-second resolve deadline, so a stalled outage cannot spend the whole deadline before the fallback is asked. On an interactive walk that skipped a provider for refusing — an open breaker, or an advertised rate-limit pause — one attempt budget is held back for each skipped provider, so a stalling fallback is cut off at its attempt budget and the skipped provider's retry has its time: the breaker's half-open trial, or the click the gate admits through a pause; a background walk, which never retries, reserves nothing. Each attempt's outcome lands on its own provider's gate.

The order is per request when a show is already known. A positive availability row names the provider whose catalogue carries the show, and a play, a download or a handoff for that show starts from it, so a show hianime listed, in the requested audio, during an anidb.app outage — a positive row a probe or a fresh resolve wrote says the show and the mode are listed there, not that a stream was played; one a cache-served replay wrote again after the row lapsed says only that a stream resolved there once validated again at its CDN — the replay stamps before anything plays, and a page warm plays nothing — and is where the next walk starts — keeps starting from hianime after anidb.app recovers — its clean miss would otherwise end the walk on a show the row says is there. So does the availability probe's own reprobe of a positive row it does not serve — a resolve's count-less stamp, an approximate count — which would otherwise start from the primary and let its clean miss overwrite the fallback's proof with a negative. The row proves the show and its mode at the show's level, not that every episode has an embed, so a remembered provider's own negative — an episode dead end, or a show found without the requested mode — does not end the walk either: the rest of the order is asked, and what it answers stands against the set-aside negative by one rank, whatever its shape — a negative that found the show outranks a title miss from another provider, clean or not — an absence and a clean miss excepted, peers since both deny and both persist — negatives of equal rank keep the one whose provider stands earliest in the configured order, and the set-aside one stands as given when the rest were unreachable. So the remembered provider's own dead end outranks the other catalogue's title miss, clean or not, and nothing persists, while its own answer that the mode is absent has denied the mode, and a negative persists when nothing outranks it — its own absence, or the other catalogue's clean miss when that provider stands earlier in the configured order, the two being peers. A remembered provider heard from with an episode dead end has not denied the show, so an absence from the rest of the order is the verdict the caller sees but not one that persists over the row, and a title miss from the rest, clean or not, is outranked by the dead end; while it is unreachable, an absence from the rest is the same verdict, seen and not persisted, and a clean miss from the rest is not the verdict at all — the remembered provider's unavailability is, the error that took it out of the walk or its gate's refusal when it was skipped and never asked, and nothing persists; whether a held absence came past an undenied affinity is read as the walk finally stands, so a remembered provider whose trial answers a clean miss makes the fallback's absence its own, and it persists. And a probe whose provider found the show but whose sampled rows all answered nothing — episode ids the provider no longer serves — is inconclusive rather than negative: the walk moves on to the next provider holding the unknown verdict it leaves, which an answer that is not negative replaces and no negative does — a title miss, clean or not, or an absence from another catalogue says nothing about the mode where this one found the show — in either order and through the skipped providers' trial; a remembered provider that gave it has not denied the show, and the verdict, when it stands, is unknown and nothing persists. And on an interactive walk the skip of an open breaker is only the fast path — the gate admits a click through an open breaker as its half-open trial — so when the walk's verdict is negative — every provider tried answered a miss or was unreachable, or the one that answered found the show without the requested mode — the skipped ones are asked before it surfaces: an answer that is not negative is the walk's; a retried provider's own negative answer or miss replaces the verdict, author and all, by the same rank as between any two negatives — the provider earliest in the configured order among equals, and never a title miss, clean or not, over the unknown verdict or a dead end that found the show, nor a clean miss over an absence or an absence over a clean miss, the two being peers; and one still unreachable leaves it standing. Background traffic keeps the skip.

The asymmetry is absence. Two verdicts persist as a negative row — a clean miss, where every search completed and nothing matched, and a show found without the requested audio mode: its episode listing is empty, as for an upcoming title, or its first readable episode lacks the mode (the probe samples one episode's server list, not the whole show's) — and each is a verdict from **the provider that answered**, which the row names: since a miss does not fail over, a negative row from anidb.app means anidb.app searched and found nothing (or found the show without the mode), not that no provider carries it. A negative row is served — by the page's probe, and by the lists, which hide a finished show on it — only while its provider has been seen answering since it last failed — its breaker closed by a success, or never opened, no failure recorded since that success, and no advertised pause running — and every provider ahead of it in the order is refusing, where refusing means the gate's breaker is open or an advertised rate-limit pause is still running; otherwise the probe runs again and the card renders. The two are different questions: past its cooldown a breaker refuses nobody but is half-open, one trial let through and nothing yet heard, so a row served on that alone would short-circuit the very probe whose trial could fail over; a half-open primary therefore backs nothing of its own, while the fallback's row yields to the trial. So anidb.app's negatives stop standing through anidb.app's own outage once its gate has learned of it — one failed request since its last success is enough, since a breaker opens only after repeated failures and the rows served meanwhile make no request that could supply them; a pause starts on an advertised rate limit — and until some request has taught the gate that, a negative row is still served without a request being made — which would otherwise hide a show hianime carries. And hianime's negatives, written during that outage so an absent show is not re-walked on every look, stand exactly as long as both halves of the read rule hold — hianime's own gate seen answering and anidb.app's refusing — so a hianime negative stops standing the moment a request to hianime itself fails, or hianime enters a pause, whatever anidb.app's gate says — and stands again when the pause's advertised window ends, on the clock, but after a failure only once a success has followed it, and after a breaker only once a success has closed it — or once a rate-limited trial has turned the open breaker into an advertised pause, which then ends on the clock like any pause; and otherwise it stands for as long as anidb.app's gate refuses: a breaker is open for its cooldown, a minute, and a pause for the window the rate limit advertised, and both close on the clock — nothing has to prove recovery — so at the window's end the next look reprobes rather than serving the row, and the reprobe's own outcome teaches the gate again, since absence on hianime proves nothing about anidb.app. A positive row's proof is guarded the same way from the other side: while the provider the row remembers is unreachable — skipped for refusing, or failed over, and not heard from since — a show found without the requested mode from the rest of the order is the verdict the caller sees but not one that persists, and a clean miss from the rest is not the verdict at all — the caller sees the remembered provider's unavailability, the error that took it out of the walk or its gate's refusal — so the row stands until the remembered provider is reachable again and the next resolve starts from it, rather than a negative from the other catalogue overwriting proof it never contradicted — for as long as the row itself is live, and no longer. Its lifetime is the [positive availability TTL](#caching), and which window it is depends on what wrote the row last. A row nothing has resolved against since the probe keeps the window the show's status earns it — 24 hours for an ongoing show, 30 days for a finished one, from that probe. Only the probe passes the status, though: a play, a download, a hand-off and a page's warm all stamp the row through the resolve path, which writes without one, so each rewrites the row as a 24-hour row dated from that operation — a finished show's 30 days shortened to a day like any other. A play, hand-off or page warm served from the resolution cache is the exception: it learned nothing about the listing — the CDN answered for a URL resolved when the provider last served — so it leaves a standing positive row exactly as it is, lifetime included, and only where no positive row stands, the resolution cache outliving the availability row, writes a count-less 24-hour row naming the provider, which the next look at the show reprobes for its cap, starting from that provider. So what bounds the remembered provider's protection is a day from the last resolve against the show, and the probe's own window only for as long as the probe is the last thing that wrote. Past that the cache stops returning the row, nothing remembers the provider, and an outage that outlasts it leaves the next walk running the plain order, where the other catalogue's negative persists like any other. A remembered provider that answers for itself is heard from, and its own negative persists as before.
...
| Availability probe (positive, finished show) | SQLite `meta_cache` | 30 days, until a resolve against the show rewrites the row at 24 hours |
...
Only the probe passes the show's status when it writes an availability row, so a negative row's lifetime, like a positive one's, depends on what wrote it last. A play, a download, a hand-off or a page's warm whose walk ended in a clean miss stamps that miss through the resolve path, which writes without a status, so the negative row it writes — over a probe's row, or where none stood — gets the window of an ongoing show whatever the show's status: 24 hours at most, dated from that operation, a finished show's seven days and an unreleased show's premiere-sized window included. The row is served as any negative row is, under the read rule above, for as long as it lasts, and the status-sized window comes back only when a probe runs and writes the row again.
...
The **availability TTL branches on Kitsu's `status` field**: shows airing weekly need a 24-hour window so a new episode surfaces within a day, but finished shows can hold for 30 days. Unknown / missing status falls back to the short (24h) window — a stale "no episode 1161 yet" is much worse than re-probing too eagerly. The long window belongs to the probe's own row: every other writer — a play, a download, a hand-off, a page's warm — stamps the row without a status, so a finished show's row is rewritten as a 24-hour one dated from that resolve; a play, hand-off or page warm served from the resolution cache writes only where no positive row stands, and leaves a standing one its own window.
