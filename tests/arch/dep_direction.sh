...
#
#   4. `frontend/src/`           imports nothing from outside its
...
if [ ! -d backend ]; then
    printf 'arch/dep_direction: backend/ does not exist yet — skipping\n'
    exit 0
fi
...
if [ -d backend/src/cache ]; then
    files=$(find backend/src/cache -type f -name '*.rs')
    if [ -n "$files" ]; then
...
if [ -d backend/src/proxy ]; then
    files=$(find backend/src/proxy -type f -name '*.rs')
    if [ -n "$files" ]; then
        # shellcheck disable=SC2086
        assert_no_match \
...
#    than it is.
if [ -d backend/src/scraper ]; then
    files=$(find backend/src/scraper -type f -name '*.rs')
    if [ -n "$files" ]; then
        # shellcheck disable=SC2086
        assert_no_match \
...
#    that escape src/, which would hit backend or worse.
if [ -d frontend/src ]; then
    files=$(find frontend/src -type f \( -name '*.ts' -o -name '*.svelte' \))
    if [ -n "$files" ]; then
        # The `\.\.\/\.\.\/\.\.` pattern catches `../../..` chains that
        # escape src/<area>/<file>.ts → src/<area>/ → src/ → frontend/.
        # shellcheck disable=SC2086
        assert_no_match \
