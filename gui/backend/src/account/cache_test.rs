...
fn upsert_entry_updates_one_row_without_touching_others() {
    ...
    // entry must be written back to the cache so the Watch Later rail
    // (which filters status === 'planning') drops a just-started show.
    // upsert_entry touches only its own (provider, user_id, media_id)
    // row — unlike write_entries, it must NOT wipe the rest of the list.
    let pool = open_in_memory().unwrap();
    write_entries(
        &pool,
        ProviderKind::AniList,
