...
- **The primary provider is unreachable** — the walk runs against the next provider. A miss reached there is persisted as that provider's negative verdict, naming it — unless a live positive row remembers the unreachable provider, in which case the miss is the verdict the caller sees but not one that persists, since the row's own provider has not denied the show and an unrelated absence must not overwrite its evidence — and a persisted one is served only while that provider's gate has been seen answering and the primary's gate is refusing — a breaker open for its cooldown, or a rate-limit pause running for its advertised window; both end on the clock, with nothing having to prove the primary is back, and at the window's end the row stops standing and the probe runs again.
