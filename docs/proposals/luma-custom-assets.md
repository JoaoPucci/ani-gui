...
1. Asset directory `frontend/static/illustrations/`, gitignored at the source level but populated by a `pnpm run assets:bake` script that hits Luma, runs through the critic agent, and saves the surviving .webp / .mp4 outputs into the directory.
