...
async fn a_download_refuses_when_the_cross_instance_lock_cannot_be_taken() {
    ...
    // and the case it protects — two instances deleting and replacing
    // each other's finished file — is the one nobody reproduces on
    // purpose. Better to refuse the download and say why.
    //
    // Refusing is only reasonable because the lock lives in a directory
    // the app owns and creates. Failing there means the cache directory
    // is unusable, which the metadata database and the image cache are
    // about to discover too.
    //
    // The obstruction is a directory standing where the lock file
    // belongs, so opening it for writing cannot succeed. That is
    // deliberately scoped to this target's own lock path: an earlier
    // version of this case blocked the whole cache directory through
    // the environment, which every download test running beside it
    // then read, and it failed them instead.
    let bin = tempfile::tempdir().expect("bin");
    let dest = tempfile::tempdir().expect("dest");
    let ran = dest.path().join("ran");
    stage_tool(
    ...
    let target = dest.path().join("Unlockable Show Episode 1.mp4");
    let lock_path = target_lock_path(&target).expect("a lock path for the target");
    std::fs::create_dir_all(&lock_path).expect("stage a directory where the lock file belongs");

    let mut sink = |_l: &str| {};
    let outcome = spawn_download_tool(
        "https://cdn.example/x/master.m3u8",
        dest.path(),
        "Unlockable Show Episode 1",
        None,
    ...
    assert!(
        outcome.is_err(),
        "a download that cannot take the cross-instance lock must refuse, not proceed unguarded"
    );
    assert!(
        !ran.exists(),
        "the tool ran even though the cross-instance lock was never held"
    );
}
