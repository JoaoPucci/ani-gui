...
mod tests {
    ...
    // ----- streaming_episodes_for_mal_id -----------------------------------
    //
    // AniList `Media.streamingEpisodes` carries Crunchyroll-style entries
    // with `title` like "Episode N - ..." and `thumbnail` URLs. The parser
    // extracts an integer episode number from each title; entries without
    // a parseable number or without a thumbnail are dropped. Used by the
    // Kitsu episodes route to backfill thumbnails where Kitsu has nulls.
