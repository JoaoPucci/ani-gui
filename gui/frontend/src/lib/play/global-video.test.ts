...
describe('canReuseSession', () => {
	it('returns the session when ids, episode, quality, mode, and state all line up', () => {
		expect(canReuseSession(session, livePlayback, 'kid-42', 5, 'best', 'sub')).toBe(session);
	});

	it('returns null when there is no current session', () => {
		// A fresh app boot, or after a teardown — nothing to reuse.
		expect(canReuseSession(null, livePlayback, 'kid-42', 5, 'best', 'sub')).toBeNull();
	});

	it('returns null when the kitsu_id differs', () => {
		expect(canReuseSession(session, livePlayback, 'kid-99', 5, 'best', 'sub')).toBeNull();
	});
	...
	it('returns null when the episode differs (next-episode click)', () => {
		...
		expect(canReuseSession(session, livePlayback, 'kid-42', 6, 'best', 'sub')).toBeNull();
	});

	it('returns null when the requested quality differs from the loaded session', () => {
		// The user went back, switched quality, and re-clicked the same
		// episode. The loaded PiP stream is still the old quality, so
		// reuse must be refused and the page must re-resolve at the new
		// quality instead of silently continuing the old one.
	...
	it('returns null when the requested mode differs from the loaded session', () => {
		...
		expect(canReuseSession(session, livePlayback, 'kid-42', 5, 'best', 'dub')).toBeNull();
	});
	...
	it('returns null when no <video> exists (state snapshot null)', () => {
		...
		expect(canReuseSession(session, null, 'kid-42', 5, 'best', 'sub')).toBeNull();
	});
	...
	it('returns null when the element has no source loaded', () => {
		...
		// empty <video>.
		expect(
			canReuseSession(session, { hasSource: false, ended: false }, 'kid-42', 5, 'best', 'sub')
		).toBeNull();
	});
	...
	it('returns null when the video has reached its end', () => {
		...
		expect(
			canReuseSession(session, { hasSource: true, ended: true }, 'kid-42', 5, 'best', 'sub')
		).toBeNull();
	});
});
...
describe('singleton lifecycle (happy-dom)', () => {
	...
	it('reuseSessionIfMatching consults the live singleton state', () => {
		...
		expect(reuseSessionIfMatching('kid-1', 1, 'best', 'sub')).toBeNull();
		setCurrentSession({
			kitsu_id: 'kid-1',
			episode: 1,
			session_id: 'sess-x',
		...
		expect(reuseSessionIfMatching('kid-1', 1, 'best', 'sub')?.session_id).toBe('sess-x');
		// And it still rejects mismatched ids…
		expect(reuseSessionIfMatching('kid-2', 1, 'best', 'sub')).toBeNull();
		expect(reuseSessionIfMatching('kid-1', 2, 'best', 'sub')).toBeNull();
		// …and a changed quality/mode, so a re-click after switching
