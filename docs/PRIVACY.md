...
- **Anime catalogue lookups** — Kitsu, AniList, MyAnimeList (the last
  ...
  can stream an episode from more than one content delivery network,
  and the sources request says which one it is answered for; the app
  asks every megaplay server for the one network whose streams it can
  play, whichever network the site's listing named that server for. So
  the hosts a megaplay page leads to are that network's, and no
  other's.
