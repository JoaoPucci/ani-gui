...
async fn the_pick_walk_reports_the_all_clean_miss() {
    ...
    let provider = ScriptedTransport::new(&[]);
    let ne = walk(
        &provider,
        "unknown show",
...
async fn the_pick_walk_recovers_through_a_later_alias() {
    ...
    let provider = ScriptedTransport::new(Box::leak(
...
async fn the_pick_walk_stops_typed_on_an_interstitial() {
    ...
    let provider = ScriptedTransport::new(&[("blocked", "!")]);
    let ne = walk(
        &provider,
        "blocked show",
...
async fn the_pick_walk_marks_answered_dead_ends_non_persistable() {
    ...
    let provider =
        ScriptedTransport::new(Box::leak(vec![("ghost", ghost_pool)].into_boxed_slice()));
