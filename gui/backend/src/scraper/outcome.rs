...
//! back. Split from the gate machine so the vocabulary reads (and
//! measures) on its own.

use tokio::time::Duration;

/// Typed result of an admitted request, replacing the bool for
/// callers that can distinguish the provider's in-band rate limit
/// (and its advertised retry hint) from an untyped failure.
...
pub enum ScrapeOutcome {
    ...
    Failure,
    /// The provider answered with its in-band rate limit. One such
